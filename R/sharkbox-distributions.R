...
#'
#' @examples
rbrokenbar = function(n, S, sequential=FALSE) {

  if(S<2) stop("S must be greater or equal than 2.")

  .rbrokenbar1 = function(n, S) {
  ...
  if(S==2) return(.rbrokenbar1(n, S))

  if(isTRUE(sequential)) return(.rbrokenbar2(n, S))

  return(.rbrokenbar1(n, S))
